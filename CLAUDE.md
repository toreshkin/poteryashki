...
выдумывает: она выбирает место из `lib/districts.ts` (районы Бишкека и города
Кыргызстана), оттуда же берутся `lat/lng`; `findDistrict` сопоставляет основы
слов с окончаниями, поэтому находит «на Дордое» и «у Ошского рынка», но не
путает «ошейник» с городом Ош. Что не нашлось в словаре, ищется геокодером
`lib/geocode.ts` (Nominatim, только Кыргызстан, лимит 1 запрос/сек и кэш).
**Нераспознанное место не заменяется центром города** — иначе объявление из
Токмока молча окажется на карте Бишкека; вместо этого место указывает человек.
